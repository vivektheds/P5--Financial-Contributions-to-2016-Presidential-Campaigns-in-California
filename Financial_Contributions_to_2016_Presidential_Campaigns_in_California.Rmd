---
title: "Exploration of Financial Contributions to Presidential Campaigns in California"
author: "Vivek Pandey"
...
print(summary(CA_FEC$contb_receipt_amt))
...
# Clean up the data and remove negative amount
...
ggplot(aes(x = contb_receipt_amt),data=CA_FEC1 )+
        geom_histogram() +
        xlab("Contribution Received Amount ($)") +
...
# Contribution Amount distribution with bandwidth
...
# Tranformed graph of Contribution Amount distribution
       
ggplot(CA_FEC1, aes(sqrt(contb_receipt_amt))) + 
 geom_histogram() + 
        scale_x_sqrt("Contribution amount square root-scaling ($)")+
        scale_y_continuous(name="No of contributors", labels = scales::comma)
...
```{r echo=FALSE, Univariate_Summary_by_Gender}
# Summary by Gender
...
```{r echo=FALSE, Univariate_Plot_by_Occupation}
...
```{r echo=FALSE, Contribution_Summary_by_party}
# Populate a dataframe for aggregated contribution amount by candidates
...
# Contribution Amount summary by party
Funding_By_party
...
# Bivariate Plot of contribution amount by each party
...
# Populate a dataframe for aggregated contribution amount by party and candidates
...
# Bivariate plot of contribution amount by each candidates and their party
...
# Populate a dataframe for percentage aggregated contribution amount by party and candidates
...
# Graph of Democratic party's percentage contributing amount by their candidates
...
# Graph of Republican party's percentage contributing amount by their candidates
...
# Populate a dataframe for aggregated contribution amount for both parties by occupation
...
# Graph of no of contributors from top 10 occupation for each party
...
plot <- plot + geom_bar(stat = "identity", position = 'dodge') +
        ...
        ggtitle("Top 10 occupation Contributors for each party") +
        coord_flip()
plot
...
# Populate a dataframe for aggregated contribution amount by gender, party and candidates
...
# Populate a dataframe for aggregated contribution amount by gender
funding_gender <- CA_FEC1 %>%
 group_by(gender) %>%
 summarise(Total_Amount = sum(contb_receipt_amt)) 

# Populate a dataframe for aggregated contribution amount by gender and party
funding_gender_party <- CA_FEC1 %>%
 group_by(gender,party) %>%
 summarise(Total_Amount = sum(contb_receipt_amt)) 

# Summary of total amount of contribution received by Gender and Party
...
# plot of total contribution amount by gender
...
#Plot of total contribution amount by gender and party
...
# Create a Data Frame Of California Map Data

map_ca = map_data('county', 'california')

# Create a data frame to store top five cities of california
...
# Plot a map for contributors in California
ggplot() + 
...
```{r echo=FALSE, BivariatePlot_by_Location_party, fig.width=9, fig.height=7}
...
# Plot a map for democratic contributors in California
plot1<- ggplot() + 
  geom_polygon(data = map_ca, aes(x = long, y = lat, group = group), 
               colour = "black", fill = "#BBDAA4") + 
...
# Plot a map for republican contributors in California
plot2<- ggplot() + 
  geom_polygon(data = map_ca, aes(x = long, y = lat, group = group), 
               colour = "black", fill = "#BBDAA4") + 
...
# show maps for both democratic and republican contributors
...
# Create new variable Mon_year to group receipt date into Month and year
CA_FEC1$Mon_year <- format(as.Date(CA_FEC1$contb_receipt_dt, "%d-%b-%y"),"%b-%y")


# Create a dataframe for aggregated contribution amount by receipt month and year
Contr_by_Mon_yyyy <- CA_FEC1 %>%
 group_by(Mon_year) %>%
 summarise(Total_Amount = sum(contb_receipt_amt)) 

# Convert mon_year variable from Mon-yyyy format to date format where day will be always first day of month so that I can use date column for time series analysis
...
# Create a dataframe for top five candidates' aggregated contribution amount by receipt month-year      
...
# Time series graph of contributing amount over course of US presidential financial campaign
...
# Plot a Time series graph of contributing amount received by top five candidates over course of US presidential financial campaign
...
# Plot a graph for contributing amount received by candidates and their party
ggplot(Funding_By_party_cand_nm, aes(cand_nm, Total_Amount,fill=party)) +
        geom_bar(stat="identity") +
...
# Create a dataframe for top five candidates' aggregated contribution amount by gender
Funding_By_Gender_cand_nm <- Funding_By_Gender_party_cand_nm %>% 
      filter(cand_nm %in% c("Clinton, Hillary Rodham", "Sanders, Bernard" ,"Trump, Donald J.","Rubio, Marco" ,"Cruz, Rafael Edward 'Ted'" ))

# Plot a graph for Top five candidates' Contribution Amount by Gender
ggplot(Funding_By_Gender_cand_nm, aes(cand_nm, Total_Amount,fill=gender)) +
        geom_bar(stat="identity", position = position_dodge()) +
        theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
        xlab("Candidate Name") +
         scale_fill_brewer(palette="Dark2") +
        ggtitle('Top five candidates Contribution Amount by Gender') +
        scale_y_continuous(name="Total Amount Received ($)", labels = scales::comma)
...
# Plot a graph for no of contributors who belongs to top 10 occupations and contributed to both party 
...
plot_profession <- plot_profession + geom_bar(stat = "identity", position = 'dodge') +
        ...
        xlab("Occupations")+
        ylab("No of Contributors") +
        ggtitle("Contribution for each party by Top 10 Occupations") +
        coord_flip()
plot_profession
...
### Struggles: The 2016 presidential campaign contributions dataset for California state contains over 289901 observations.
...
### Conclusion: After analyzing California financial contribution data I came across many interesting findings and patterns.
